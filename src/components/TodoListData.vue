<template>
  <ul class="tab">
    <li
      :class="{ active: currentState == state.title }"
      @click="currentState = state.title"
...
const getApiTodoList = async () => {
  const res = await TodoList.getAllTodoList()
  if (res) {
    console.log('getApiTodoList', res)
    todoList.value = res
  }
}
...
const visibleTodoList = ref<TodoListType[]>([])
...
const deleteTodoList = async (id: number | undefined) => {
  await axios
    .delete(`https://type-script-demo.vercel.app/todoList/${id}`)
    .then((res) => {
...
//isCompleted
const isCompleted = async (list: TodoListType) => {
  list.isCompleted = !list.isCompleted
  await axios
    .patch(`https://type-script-demo.vercel.app/todoList/${list.id}`, list)
    .then((res) => {
