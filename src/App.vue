<template>
  <div class="container">
    ...
    <div class="card card_list">
      <todo-list-data @deleteCompleted="deleteCompleted" :getTodoListNum="getTodoListNum" :todoList="todoList"
        @getTodoList="getTodoList"></todo-list-data>
    </div>
  </div>
</template>
...
const getTodoList = () => {
  axios.get("https://type-script-demo.vercel.app/todoList")
    .then((res) => {
...
const addList = async () => {
  ...
  if (textInput.value !== "") {
    await axios.post("https://type-script-demo.vercel.app/todoList", todoList)
      .then((res) => {
...
const deleteCompleted = async () => {
  todoList.value.filter((item) => {
    if (item.isCompleted === true) {
      axios.delete(`https://type-script-demo.vercel.app/todoList/${item.id}`)
        .then((res) => {
          console.log(res.data);
          getTodoList()
        })
